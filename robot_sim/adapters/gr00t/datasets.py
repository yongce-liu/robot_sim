...
class Gr00tDatasetsBuilder:
    ...
    def _default_max_workers(self, task_count: int) -> int:
        cpu = os.cpu_count() or 4
        return max(1, min(32, cpu + 4, task_count))
    ...
    def output_data(self, suffix: str = "parquet"):
        ...
        def _build_and_write(ep_idx: int, records: list[SingleRecord]):
            output_name = output_format.format(
                **{self.chunk_key: ep_idx // self.chunk_size, self.episode_key: ep_idx % self.chunk_size}
            )
            data = []
        ...
        items = list(self.src.items())
        with ThreadPoolExecutor(max_workers=self._default_max_workers(len(items))) as executor:
            futures = [executor.submit(_build_and_write, ep_idx, records) for ep_idx, records in items]
    ...
    def output_video(self, suffix: str = "mp4"):
        ...
        def _build_and_write(video_key: str, ep_idx: int, records: list[SingleRecord]):
            output_name = output_format.format(
                **{
                    self.chunk_key: ep_idx // self.chunk_size,
                    "video_key": video_key,
        ...
        with ThreadPoolExecutor(max_workers=self._default_max_workers(len(items))) as executor:
            futures = [
                executor.submit(_build_and_write, video_key, ep_idx, records) for video_key, ep_idx, records in items
            ]
            for future in futures:
                future.result()
